...
class HuobiUsdtSwapTrade(Websocket):
    ...
    async def get_swap_lightning_close_position(self, quantity, action):
        """ 闪电平仓
        """
        ...
        if action == ORDER_ACTION_BUY:
            ...
            return None, "action error"
        result, error = await self._rest_api.swap_lightning_close_position(self._symbol, quantity, direction)
        if error:
            return None, error
        return result, error
